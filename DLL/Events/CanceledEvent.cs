...
namespace ORM_Monitor.Events {
  ...
  public sealed class CanceledEvent<T> : TaskHandler<T>, ICanceledEvent<T> {
    ...
    public CanceledEvent(CancellationTokenSource ctx, TaskEventArgs.Expression expression) : base(ctx, expression) {
    }
    ...
    ///   OnCanceled
    /// </summary>
    public event EventHandler<TaskEventArgs> OnCanceled {
      add {
        lock (Mutex) {
          Handler += value;
        }
