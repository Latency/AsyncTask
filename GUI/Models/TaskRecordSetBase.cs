    ...
    public abstract class TaskRecordSetBase : DependencyObject
    {
        ...
        protected TaskRecordSetBase()
        {
            ...
            foreach (var prop in typeof(TaskRecordSet).GetProperties())
            {
                ...
                DependancyCollection.Add(prop.Name,
                                         DependencyProperty.Register(prop.Name, propertyType, GetType(),
                                                                     new UIPropertyMetadata(propertyType.IsValueType ? Activator.CreateInstance(propertyType) : null,
                                                                                            (sender, e) => sender.InvokeIfRequired(objs => { typeof(DispatcherObject).GetProperty((string) objs[1])?.SetValue(objs[0], objs[2]); },
        ...
        /// <returns></returns>
        protected T Get<T>(string property) => (T) this.InvokeIfRequired(objs =>
        {
            lock (Mutex)
            {
        ...
        protected void Set<T>(string property, T value) => this.InvokeIfRequired(objs =>
        {
            lock (Mutex)
            {
                if (!(objs[0] is string key))
                    return;
                var v = objs[1] is T variable ? variable : default;
